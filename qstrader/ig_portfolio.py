...
class Portfolio(object):
    ...
    def _update_portfolio(self):
        ...
        for ticker in self.positions:
            ...
            self.logger.debug('Updating market value with bid: %s and ask: %s' % (bid / PriceParser.PRICE_MULTIPLIER, ask / PriceParser.PRICE_MULTIPLIER))
            self.unrealised_pnl += pt.unrealised_pnl
            self.equity += (
                pt.market_value - pt.cost_basis + pt.realised_pnl
            )
            self.logger.debug('Unrealised PnL: %s' % (float(self.unrealised_pnl) / PriceParser.PRICE_MULTIPLIER))
            self.logger.debug('Realised PnL: %s' % (float(self.realised_pnl) / PriceParser.PRICE_MULTIPLIER))
            self.logger.debug('Equity: %s' % (float(self.equity) / PriceParser.PRICE_MULTIPLIER))
